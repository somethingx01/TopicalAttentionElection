...
ROOT_DIR = '/media/data1/lixing/2017_2018_2_Aston/TopicalAttentionElection'

TRAIN_SET_PATH = '%s/datasets/ElectionTweets.snstrain_10.wordembbed.csv'%ROOT_DIR
TEST_SET_PATH = '%s/datasets/ElectionTweets.snstrain_00.wordembbed.csv'%ROOT_DIR

SAVE_DIR = '%s/save'%ROOT_DIR

WORD_EMBEDDING_DIMENSION = 300 #must set before run
TOPIC_EMBEDDING_DIMENSION = 9
#ALPHABET_SIZE=len(ALPHABET)
#DICT = {ch: ix for ix, ch in enumerate(ALPHABET)}
#CLASS_LEVEL1 = 17 #must set before run
#CLASS_LEVEL2 = 12 #must set before run
TWITTER_LENGTH = 23 #universal twitter length for each twitter, must set before run
USER_SELF_TWEETS = 3 #a user's previous tweet nums, must set before run
NEIGHBOR_TWEETS = 4 #neighbors' previous tweet nums, must set before run
TRAINING_INSTANCES = 19676 #19676
TESTING_INSTANCES = 15787

# CLASS_COUNT = 3 # number of classes for classification  
TOPIC_SENTIMENT_COUNT = 8
SENTIMENT_COUNT = 3

CHUNK_SIZE_MULTIPLIER_BATCH_SIZE = 2 # CHUNK_SIZE = CHUNK_SIZE_MULTIPLIER_BATCH_SIZE * BATCH_SIZE
CONST_TWEET_WEIGHT_A = 100.0 # 1000 normal
...
class DefaultConfig():
    ...
    '''
    batch_size = 512 #1024 # 256 best for 01 #if if 2048 then cuda memory exceeds
    epochs = 100 # 200 normal
    learning_rate = 0.005 # 0.0005 initial best, learning rate initialize should depend on the batch size
    lr_decay = 0.9 #0.95 # 0.93 0.005 best # 0.9 #0.6 better
    weight_decay = 1e-4
