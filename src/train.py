...
os.environ["CUDA_VISIBLE_DEVICES"]="1" #cuda device id
    ...
    **kwargs: train(a=1,b=2,c=3,d=4)CustomPreProcessor=>kwargs[a] = 1, kwargs[b] = 2, kwargs[c] = 3, kwargs[d] = 4, kwargs is principally a dict
    ...
    for epoch in range( epochs ):
        ...
        if acc > best_acc:
            ...
            if epoch <= 5:
                lr = lr * config.lr_decay # only lr = lr * config.lr_decay can be higher as 0.72629! what amazing!
            else:
                if epoch <= 10:
                    lr = lr * ( config.lr_decay +  0.05 * config.lr_decay)
                else:
                    if epoch <= 50:
                        lr = lr * ( config.lr_decay + 0.07 * config.lr_decay) # 0.95
                    else:
                        if epoch <= 70:
                            lr = lr * ( config.lr_decay + 0.085 * config.lr_decay) # 0.97
                        else:
                            if epoch <= 150:
                                lr = lr * ( config.lr_decay + 0.099 * config.lr_decay) # 0.99
                            else:
                                lr = lr * ( config.lr_decay + 0.05 * config.lr_decay) # 0.99
...
if __name__=='__main__':
    ...
    # prediction dosen't requires previous epoch
    #predict('%s/35'%SAVE_DIR) # best: 22 # 6 election small dataset # 10 election larger dataset
    calculate_accuracy('%s/35'%SAVE_DIR) # 6 election small dataset